...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("رفع ملف (CSV/Excel)", type=["csv", "xlsx"])
    ...
    with c1:
        if st.button("🚀 قياسي"):
            ...
            uploaded_file = pd.DataFrame(sample)
    with c2:
        if st.button("🧪 نانو"):
            nano_sample = {
            ...
            uploaded_file = pd.DataFrame(nano_sample)
...
def calc_thalf(time, conc):
    try:
        log_c = np.log(conc[conc > 0][-3:]) # تلافي القيم الصفرية
        t_subset = time[conc > 0][-3:]
...
if uploaded_file is not None:
    ...
    st.success(f"✅ تم تفعيل تحليل البروتوكول: {study_id}")

    # ملخص الدراسة
    st.markdown(f"""
    <div class='info-card'>
        <strong>ملخص المكونات الرقابي:</strong><br>
    ...
    with col_graph:
        st.subheader("📊 ملف الحركية الدوائية (PK Profile)")
        ...
        ax.set_title(f"Bioequivalence Study: {drug_api}", fontsize=14)
        ax.legend()
    ...
    with col_stats:
        ...
        for col in df.columns[1:]:
            ...
            res_list.append({'Brand': brand, 'Cmax': cmax, 'Tmax': tmax, 'AUC': auc, 't½': thalf})
        ...
        if len(res_list) >= 2:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.metric("نسبة AUC T/R", f"{auc_ratio:.2f}%")
            with c2:
                st.metric("نسبة Cmax T/R", f"{cmax_ratio:.2f}%")
            
            if 80 <= auc_ratio <= 125 and 80 <= cmax_ratio <= 125:
                st.success("✅ التركيبة متكافئة حيوياً (Passed)")
            else:
                st.error("❌ التركيبة غير متكافئة حيوياً (Failed)")

    # تصدير التقرير
    report = pd.DataFrame(res_list)
    csv_data = report.to_csv(index=False).encode('utf-8-sig')
    st.download_button("📥 تحميل التقرير النهائي للرقابة (CSV)", csv_data, f"Regulatory_Report_{study_id}.csv", "text/csv")

else:
    st.info("👋 مرحباً بك في منصة Sama Pharma Tech. يرجى اختيار مادة التحميل والضغط على 'بيانات نانو' للمشاهدة.")
...
st.divider()
st.caption(f"Sama Pharma Tech | نظام التحليل الرقمي الإصدار 2.0 | API: {drug_api if drug_api else 'غير محدد'}")